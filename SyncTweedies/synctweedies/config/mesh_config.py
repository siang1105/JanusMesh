...
def load_mesh_config():
    ...
    parser.add_argument('--secondary_prompt', type=str, default=None, help='Optional prompt that can be assigned to a subset of views')
    parser.add_argument('--secondary_negative_prompt', type=str, default='oversmoothed, blurry, depth of field, out of focus, low quality, bloom, glowing effect.', help='Optional negative prompt for the secondary prompt; defaults to --negative_prompt when omitted')
    parser.add_argument('--tertiary_prompt', type=str, default=None, help='Optional third prompt for use with azim_split_mode="third"')
    parser.add_argument('--tertiary_negative_prompt', type=str, default=None, help='Optional negative prompt for the tertiary prompt; defaults to --negative_prompt when omitted')
    parser.add_argument('--azim_split_mode', type=str, default='quadrant', choices=['quadrant', 'half', 'third'], help='Azimuth split mode: "quadrant" (270-360,0-90 vs 90-270) or "half" (0-180 vs 180-360) or "third" (0-120 vs 120-240 vs 240-360)')
    parser.add_argument('--prompt_transition_width', type=float, default=0.0, help='Width of transition region in degrees for blending between prompts at boundaries (0.0 = no transition, hard switch)')
    parser.add_argument('--prompt_transition_extension', type=float, default=2.0, help='Extension factor for transition width. Effective blending range = transition_width * extension (default: 2.0, meaning 2x the transition_width for smoother blending)')
